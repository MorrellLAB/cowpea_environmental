#!/usr/bin/env Rscript
# R. Akakpo - adapted from C. Fiscius
# UMN Saint Paul 
#08/10/2023
# This script is used to extract Bioclim data from wordclim 2 .tiff files
# Need .tiff files to be previously dowload from Wordclim
...
df<-read.table("./Data/Sample_cordinates.txt", h=T, quote = "", sep="\t")
df<-as.data.frame(df)
...
# compile wc data
files<-list.files(path="~/MorrellLab Dropbox/Roland Akakpo/Cowpea_Environmental_assoc/envGWAS/Data/Pheno_data/wc2.1_bio/", pattern=".tif", full.names=T)
